...
const COMPTES_AFFICHES = ['Caisse','BATG','BOA','BSIC'];
...
export default function TresoreriePage() {
  ...
  const charger = async () => {
    try {
      ...
      // Filtrer pour n'afficher que les 4 comptes principaux
      const filtres = c.filter(x => {
        const lbl = (x.libelle||x.nom||'').toLowerCase();
        return lbl.includes('caisse') || lbl.includes('batg') || lbl.includes('boa') || lbl.includes('bsic');
      });
      setComptes(filtres);
  ...
  const saveMvt = async () => {
    ...
    try {
      await tresorerieAPI.ajouterMouvement({
        compte_id:form.compte_id, sens:typeModal,
        montant_fcfa:+form.montant, date_mouvement:form.date,
        description:form.libelle, type_operation:form.type_operation,
      });
  ...
  // Onglets : Tous + les 4 comptes
  const TABS = [
    {key:'all', label:'Tous comptes'},
    ...comptes.map(c=>({key:c.id, label: c.libelle?.includes('Caisse')?'Caisse': c.libelle?.includes('BOA')?'BOA': c.libelle?.includes('BSIC')?'BSIC': c.libelle?.includes('BATG')?'BATG': c.libelle}))
  ];
    ...
    <div className="fade-up">
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:10}}>
        {comptes.map((c,i)=>(
          <div key={i} className="kpi" style={{borderTop:`2px solid ${CCOLORS[i%4]}`,cursor:'pointer'}}
            ...
            <div className="kpi-val" style={{fontSize:16,color:CCOLORS[i%4]}}>{fmt(c.solde_fcfa||c.solde||0)}</div>
            <div className="kpi-sub kn">FCFA</div>
            <div className="kbar">
              <div className="kbar-f" style={{width:`${Math.round(Math.abs(parseFloat(c.solde_fcfa||0))/maxS*100)}%`,background:CCOLORS[i%4]}}/>
            </div>
          </div>
        ))}
      </div>
      ...
      {isDGwrite&&(
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:14}}>
          <button className="btn success" style={{justifyContent:'center',padding:'11px',fontSize:13,fontWeight:600}}
            onClick={()=>ouvrirModal('credit')}>
            ↑ Enregistrer une entrée d'argent
          </button>
          <button className="btn danger" style={{justifyContent:'center',padding:'11px',fontSize:13,fontWeight:600}}
            onClick={()=>ouvrirModal('debit')}>
            ↓ Enregistrer une sortie d'argent
          </button>
        </div>
      )}

      {/* Onglets filtre + filtre type */}
      <div style={{display:'flex',gap:8,marginBottom:12,alignItems:'center',flexWrap:'wrap'}}>
        <div style={{display:'flex',gap:4,background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:9,padding:3}}>
          {TABS.map(t=>(
            <button key={t.key} className={`treso-tab${tabActif===t.key?' active':''}`}
              onClick={()=>setTabActif(t.key)}>
              {t.label}
            </button>
          ))}
        </div>
      ...
      {/* Brouillard + graphique flux 6 mois */}
      <div style={{display:'grid',gridTemplateColumns:'1.6fr 1fr',gap:12,marginBottom:12}}>
        <div className="card">
          <div className="card-hd">
            <div className="card-t">
              {tabActif==='all'?'Brouillard de caisse — Tous comptes'
                :`Relevé — ${comptes.find(c=>c.id===tabActif)?.libelle||''}`}
            </div>
        ...
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setModal(false)}>
          <div className="modal" style={{width:480}}>
            ...
            <div className="form-grp" style={{marginBottom:12}}>
              ...
              <select className="form-sel" style={{width:'100%'}} value={form.compte_id}
            ...
            <div className="form-grp" style={{marginBottom:12}}>
              ...
              <select className="form-sel" style={{width:'100%'}} value={form.type_operation}
                onChange={e=>setForm(f=>({...f,type_operation:e.target.value}))}>
            ...
            <div className="form-row">
              <div className="form-grp">
                ...
                <input type="number" className="form-inp" min={1} value={form.montant}
            ...
            <div className="form-grp" style={{marginBottom:16}}>
              ...
              <input type="text" className="form-inp" value={form.libelle}
